...
class Log extends DataClass implements Insertable<Log> {
  ...
  @override
  int get hashCode => Object.hash(id, category, supplement, registeredAt,
      amount, imageUrl, confirmed, updatedAt);
  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      (other is Log &&
...
class $SchedulesTable extends Schedules
  ...
  late final GeneratedColumn<int> monthlyHeadOrigin = GeneratedColumn<int>(
      'monthly_head_origin', aliasedName, true,
      type: DriftSqlType.int, requiredDuringInsert: false);
  ...
  late final GeneratedColumn<int> monthlyTailOrigin = GeneratedColumn<int>(
      'monthly_tail_origin', aliasedName, true,
      type: DriftSqlType.int, requiredDuringInsert: false);
  static const VerificationMeta _periodBeginMeta =
      const VerificationMeta('periodBegin');
  @override
...
class Schedule extends DataClass implements Insertable<Schedule> {
  ...
  Map<String, Expression> toColumns(bool nullToAbsent) {
    ...
    if (!nullToAbsent || monthlyHeadOrigin != null) {
      map['monthly_head_origin'] = Variable<int>(monthlyHeadOrigin);
    }
    if (!nullToAbsent || monthlyTailOrigin != null) {
  ...
  SchedulesCompanion toCompanion(bool nullToAbsent) {
    return SchedulesCompanion(
      ...
      monthlyHeadOrigin: monthlyHeadOrigin == null && nullToAbsent
          ? const Value.absent()
          : Value(monthlyHeadOrigin),
      monthlyTailOrigin: monthlyTailOrigin == null && nullToAbsent
          ? const Value.absent()
          : Value(monthlyTailOrigin),
      periodBegin: periodBegin == null && nullToAbsent
          ? const Value.absent()
          : Value(periodBegin),
      periodEnd: periodEnd == null && nullToAbsent
  ...
  Schedule copyWith(
      ...
      Schedule(
        ...
        monthlyHeadOrigin: monthlyHeadOrigin.present
            ? monthlyHeadOrigin.value
            : this.monthlyHeadOrigin,
        monthlyTailOrigin: monthlyTailOrigin.present
            ? monthlyTailOrigin.value
            : this.monthlyTailOrigin,
        periodBegin: periodBegin.present ? periodBegin.value : this.periodBegin,
        periodEnd: periodEnd.present ? periodEnd.value : this.periodEnd,
      );
...
class EstimationCache extends DataClass implements Insertable<EstimationCache> {
  ...
  EstimationCache copyWith({int? category, double? amount}) => EstimationCache(
        category: category ?? this.category,
        amount: amount ?? this.amount,
      );
...
class EstimationCachesCompanion extends UpdateCompanion<EstimationCache> {
  ...
  EstimationCachesCompanion copyWith(
      {Value<int>? category, Value<double>? amount}) {
    return EstimationCachesCompanion(
      category: category ?? this.category,
      amount: amount ?? this.amount,
    );
...
abstract class _$AppDatabase extends GeneratedDatabase {
  ...
  late final EstimationAccessor estimationAccessor =
      EstimationAccessor(this as AppDatabase);
  late final ScheduleAccessor scheduleAccessor =
      ScheduleAccessor(this as AppDatabase);
  late final DisplayAccessor displayAccessor =
      DisplayAccessor(this as AppDatabase);
  late final LogAccessor logAccessor = LogAccessor(this as AppDatabase);
  late final EstimationCacher estimationCacher =
      EstimationCacher(this as AppDatabase);
  late final RepeatCacher repeatCacher = RepeatCacher(this as AppDatabase);
  late final EstimationContent estimationContent =
      EstimationContent(this as AppDatabase);
  late final RecordCollector recordCollector =
      RecordCollector(this as AppDatabase);
  @override
...
final class $$CategoriesTableReferences
  ...
  static MultiTypedResultKey<$LogsTable, List<Log>> _logsRefsTable(
      ...
      MultiTypedResultKey.fromTable(db.logs,
          aliasName: $_aliasNameGenerator(db.categories.id, db.logs.category));
...
final class $$LogsTableReferences
  ...
  static $CategoriesTable _categoryTable(_$AppDatabase db) => db.categories
      .createAlias($_aliasNameGenerator(db.logs.category, db.categories.id));
              ...
              addJoins: <
                ...
                if (category) {
                  state = state.withJoin(
                    ...
                    referencedTable: $$LogsTableReferences._categoryTable(db),
                    referencedColumn:
                        $$LogsTableReferences._categoryTable(db).id,
                  ) as T;
                }
