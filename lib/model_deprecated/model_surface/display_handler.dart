...
import 'package:miraibo/model_deprecated/transaction/display_ticket.dart';

class DisplayHandler {
  Future<void> save(view_obj.Display dt) async {
  ...
  Future<void> delete(view_obj.Display dt) async {
    if (dt.id == null) return;
    await DeleteDisplayTicket(dt.id!).execute();
  }

  Future<int> calculate(view_obj.Display dt) async {
  ...
  Future<List<view_obj.Display>> belongsTo(DateTime date) async {
    var displayTickets = await FetchDisplayTicketsBelongsTo(date).execute();
    List<view_obj.Display> ret = [];
